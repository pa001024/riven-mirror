 ...
 */
const FFT = function(bufferSize: number, sampleRate: number, windowFunc: string, alpha: number) {
  ...
  switch (windowFunc) {
    ...
    case "blackman":
      ...
      for (i = 0; i < bufferSize; i++) {
        this.windowValues[i] =
          (1 - alpha) / 2 - 0.5 * Math.cos((Math.PI * 2 * i) / (bufferSize - 1)) + (alpha / 2) * Math.cos((4 * Math.PI * i) / (bufferSize - 1));
      }
      break;
    case "cosine":
      for (i = 0; i < bufferSize; i++) {
