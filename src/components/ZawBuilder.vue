    ...
    <ul class="partlist" v-if="part === 0">
      <li v-for="item in strikeList" :key="item.id">
        <el-radio class="part" v-model="strike" :label="item" border>
          <div class="snapshot">
            <img :src="`/static/images/zawStrike${item.id.replace(/ /g, '')}.png`" :alt="item.id" width="100%">
          </div>
          <div class="name">
    ...
    <ul class="partlist" v-if="part === 1">
      <li v-for="item in gripList" :key="item.id">
        <el-radio class="part" v-model="grip" :label="item" border>
          <div class="snapshot">
            <img :src="`/static/images/zawGrip${item.id.replace(/ /g, '')}.png`" :alt="item.id" width="100%">
          </div>
          <div class="name">
            {{$t('zh') ? item.name : item.id}}
          </div>
    ...
    <ul class="partlist" v-if="part === 2">
      <li v-for="item in linksList" :key="item.id">
        <el-radio class="part" v-model="links" :label="item" border>
          <div class="snapshot">
            <img :src="`/static/images/zawLink${item.id.replace(/II/g, '2').replace(/ /g, '')}.png`" :alt="item.id" width="100%">
          </div>
          <div class="name">
            {{$t('zh') ? item.name : item.id}}
          </div>
